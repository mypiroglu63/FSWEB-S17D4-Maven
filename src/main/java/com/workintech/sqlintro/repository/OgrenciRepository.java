...
public interface OgrenciRepository extends JpaRepository<Ogrenci, Long> {
    ...
    String QUESTION_5 = "SELECT * FROM ogrenci WHERE ogrenci_numarasi BETWEEN 5 AND 10 AND cinsiyet = 'K'";
    ...
    String QUESTION_7 = "SELECT * FROM ogrenci WHERE sinif = '10A' ORDER BY ogrenci_numarasi DESC";
    @Query(value = QUESTION_7, nativeQuery = true)
    List<Ogrenci> find10AStudentsByOgrNo();

    // Öğrenciler tablosundaki en genç öğrenciyi listeleyiniz.
    String QUESTION_8 = "SELECT * FROM ogrenci ORDER BY dogum_tarihi DESC LIMIT 1";
    @Query(value = QUESTION_8, nativeQuery = true)
    Ogrenci findYoungestStudent();

    // Öğrenciler tablosundaki en yaşlı öğrenciyi listeleyiniz.
    String QUESTION_9 = "SELECT * FROM ogrenci ORDER BY dogum_tarihi ASC LIMIT 1";
    @Query(value = QUESTION_9, nativeQuery = true)
    Ogrenci findElderStudent();

    // İkinci harfi E olan öğrencileri listeleyiniz.
    String QUESTION_10 = "SELECT * FROM ogrenci WHERE ad LIKE '_e%'";
    @Query(value = QUESTION_10, nativeQuery = true)
    List<Ogrenci> findStudentsSecondLetterOfE();
